...
namespace WestWindSystem{
   ...
   //extension methods must belong to a static class
    ...
    //you can add any extension must for and class that exists within C#
       ...
       {
       //the first parameter of your method referes to the class you are attempting to             //   extend            //syntac for the firat parameter:  this theclassbeingextend parametername            //any additinal arguments exsiting on the callin statement follow the             //   first parameter separated by commas            //add the context class of your application library (DAL) to the service            //  collection            //pass in the connection string options            services.AddDbContext<WestWindContext>(options);
       //add any business logic layer class to the service collection
       //so oour            web app has access to the methods within the BLL class            
